namespace IRCd.Tests
{
    ...
    public sealed class RestartHandlerTests
    {
        ...
        [Fact]
        public async Task Restart_OperWithClassWithoutCapability_Gets481()
        {
            ...
            var h = new RestartHandler(opts, life, audit);

            var s = new TestSession { Nick = "nick", UserName = "user", IsRegistered = true };

            await h.HandleAsync(s, new IrcMessage(null, "RESTART", new string[0], null), state, CancellationToken.None);
        ...
        [Fact]
        public async Task Restart_NetAdmin_StopsApplication()
        {
            var state = new ServerState();
            ...
            Assert.Contains(s.Sent, l => l.Contains("NOTICE nick :Server is restarting"));
            Assert.Equal(1, audit.CallCount);
            Assert.Equal("RESTART", audit.Action);
