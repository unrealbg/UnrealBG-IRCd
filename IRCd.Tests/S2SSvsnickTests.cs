namespace IRCd.Tests
{
    ...
    public sealed class S2SSvsnickTests
    {
        ...
        [Fact]
        public async Task IncomingSvsnick_ChangesRemoteNickAndBroadcasts()
        {
            ...
            Assert.True(link.UserSyncEnabled);
            Assert.True(state.TryGetRemoteServerBySid("002", out var rs) && rs is not null, string.Join("\n", link.Outgoing));

            // Ensure the second link is also up so we can observe forwarding.
            ...
            var ts = ChannelTimestamps.NowTs() + 10;

            Assert.True(ServerLinkParser.TryParse($"SVSNICK abcd1234 002 002AAAAAA {ts} NewNick", out var parsedCmd, out var parsedArgs, out var parsedTrailing));
            Assert.Equal("SVSNICK", parsedCmd);
            Assert.Equal(new[] { "abcd1234", "002", "002AAAAAA", ts.ToString(), "NewNick" }, parsedArgs);
            Assert.Null(parsedTrailing);

            link.Enqueue($"SVSNICK abcd1234 002 002AAAAAA {ts} NewNick");
            link.Enqueue("UMODE cafebabe 002 002AAAAAA +i");
            ...
            Assert.True(finalUser!.Modes.HasFlag(UserModes.Invisible), $"UMODE not applied; nick={finalUser.Nick}");

            // Forwarding control: UMODE should get forwarded to the other link.
            Assert.Contains(link2.Outgoing, l => l.StartsWith("UMODE cafebabe 002 002AAAAAA ", StringComparison.OrdinalIgnoreCase));

            // If SVSNICK is processed, it should also be forwarded.
            Assert.Contains(link2.Outgoing, l => l.StartsWith("SVSNICK abcd1234 002 002AAAAAA ", StringComparison.OrdinalIgnoreCase));

            Assert.Equal("NewNick", finalUser.Nick);

            Assert.True(state.TryGetChannel("#c", out var ch) && ch is not null);
            Assert.Contains(ch!.Members, m => m.ConnectionId == "uid:002AAAAAA" && m.Nick == "NewNick");

            Assert.Contains(peerSess.Sent, l => l.Contains(":Nick!u@h2 NICK :NewNick"));
