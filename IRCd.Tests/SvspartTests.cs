namespace IRCd.Tests
{
    ...
    public sealed class SvspartTests
    {
        ...
        [Fact]
        public async Task Svspart_LocalUser_PartsAndBroadcasts()
        {
            ...
            await h.HandleAsync(operSession, new IrcMessage(null, "SVSPART", new[] { "victim", "#c" }, "bye"), state, CancellationToken.None);

            Assert.True(state.TryGetChannel("#c", out var ch) && ch is not null);
            Assert.DoesNotContain(ch!.Members, m => m.ConnectionId == "victim");

            Assert.Contains(peerSession.Sent, l => l.Contains(":victim!v@h2 PART #c :bye"));
            Assert.Contains(victimSession.Sent, l => l.Contains(":victim!v@h2 PART #c :bye"));
        }

        [Fact]
        public async Task Svspart_RemoteUser_RoutesToOwningServer()
        {
            var state = new ServerState();
            var sessions = new FakeSessionRegistry();
            var routing = new RoutingService(sessions, new IrcFormatter());
            ...
            Assert.Contains(link.Outgoing, l => l.StartsWith("SVSPART ", StringComparison.OrdinalIgnoreCase) && l.Contains(" 002AAAAA ") && l.Contains(" #c ") && l.Contains(" :bye"));
