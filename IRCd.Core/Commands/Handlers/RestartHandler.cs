namespace IRCd.Core.Commands.Handlers
{
    ...
    public sealed class RestartHandler : IIrcCommandHandler
    {
        ...
        public async ValueTask HandleAsync(IClientSession session, Protocol.IrcMessage msg, ServerState state, CancellationToken ct)
        {
            ...
            if (!state.TryGetUser(session.ConnectionId, out var user) || user is null || !OperCapabilityService.HasCapability(_options.Value, user, "restart"))
            {
